...
- 已进行针对docker的UTC时间到中国时间的转换
#### 详情页面
- 返回首页后对原筛选数据进行持久化
- 可下载录音或在线播放

#### 数据库、服务器、docker、nginx配置
- 使用了config.js和.env文件两种形式进行配置
- docker：前端宿主机8888端口映射到docker710，后端宿主机3000映射到docker3000
- nginx：将发送到8888的/api/请求转发到host.docker.internal:3000
___ 
